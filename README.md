歡迎來到 **健康廣場**🥗｜下班減脂免煩惱，健康美味一鍵搞定！  
這裡有專為忙碌上班族設計的控卡減脂食譜，讓您天天吃得美味不單調。  
**想親自下廚？** 清晰步驟帶您快速上手，下班洗切備料不心累。  
**加班太疲憊？** 一鍵下訂同款減脂餐，新鮮直送到家、開箱即食超方便。  
我們的目標，是讓您用最省時無負擔的方式，吃出人人稱羨精實好身材。

---

## 🎞️ 示範影片

| 🖥️ 桌面版體驗 | 📱 行動版體驗 |
| :---: | :---: |
| <video src="https://github.com/user-attachments/assets/7c738d07-7ac0-4aa0-a83c-61c9aa8e05eb" width="100%" controls></video> | <video src="https://github.com/user-attachments/assets/f5baad3c-aa19-4c08-ba2a-21dcaacc93dc" width="100%" controls></video> 

---

## 🌟 專案功能

- 🥦 1. 精選餐點與食譜
- 🛒 2. 購物車系統
- 🔍 3. 訂單追蹤與搜尋

---

## 🛠️ 專案環境

- **建構工具**：Vite
- **前端核心**：React
- **頁面跳轉**：React Router
- **樣式設計**：Tailwind CSS
- **狀態管理**：React Redux, Redux Toolkit
- **模擬 API 伺服器**：JSON Server

---

## 📂 專案資料夾結構

```text
data/
├── healthPlaza.json        # 預設餐點與訂單資料
src/
├── 🧩 components/          # 通用 UI 元件
│   ├── Button.jsx          # 通用按鈕元件
│   ├── Error.jsx           # 錯誤提示頁面
│   ├── Footer.jsx          # 頁尾版權聲明元件
│   ├── Header.jsx          # 頁首導覽列
│   ├── Loader.jsx          # 載入中動畫元件
│   ├── Serving.jsx         # 數量加減控制元件
│   └── Toast.jsx           # 訊息提示元件
├── 🍱 features/            # 依功能劃分之元件與邏輯
│   ├── cart/               # 購物車功能模組
│   │   ├── Cart.jsx        # 購物車頁面
│   │   ├── CartItem.jsx    # 購物車單項商品元件
│   │   ├── CartList.jsx    # 購物車列表元件
│   │   ├── CartOwner.jsx   # 訂購資料表單
│   │   ├── cartSlice.js    # 購物車狀態與 Action 控制
│   │   └── EmptyCart.jsx   # 購物車空狀態顯示元件
│   ├── menu/               # 菜單功能模組
│   │   ├── Doit.jsx        # 美食食譜頁面
│   │   └── Menu.jsx        # 美食介紹頁面
│   │   └── MenuItem.jsx    # 美食單項餐點元件
│   └── order/              # 訂單功能模組
│       ├── Order.jsx       # 訂單詳細資訊頁面
│       └── SearchOrder.jsx # 搜尋訂單元件
├── 📖 pages/               # 路由對應的頁面與主要 Layout
│   ├── AppLayout.jsx       # 整體版型配置
│   ├── Contact.jsx         # 聯絡我們頁面
│   └── Home.jsx            # 首頁組件
├── 🛠️ services/            # API 請求與非同步服務邏輯
│   └── apiHealthPlaza.js   # 健康廣場 API
├── 🚀 App.jsx              # 主路由配置
├── 🎨 index.css            # 全域樣式
├── 📌 main.jsx             # 程式進入點
└── 🏪 store.js             # 全域狀態設定
```

---

## ⚡ 啟動專案

前置作業：安裝 [Node.js](https://nodejs.org)。

### 1. 複製專案

```bash
git clone https://github.com/kiss35195/health-plaza.git
cd health-plaza
```

### 2. 安裝套件

```bash
npm i
```

### 3. 開啟本地開發伺服器

```bash
npm run server
npm run dev
```

---

## 📈 未來規劃

- [ ] 導入 React Query

---

## ⚠️ 免責聲明

- **技術展示用途**：本網站僅作為個人技術練習與展示，所有食譜、餐點及購物車下單功能**皆為虛構模擬資料**，並不會產生真實的訂單、扣款或配送服務，且無任何商業營利行為。
- **圖片版權**：網站內使用之圖片皆引用自 [Unsplash](https://unsplash.com) 免費授權圖庫，版權歸原創作者所有。
